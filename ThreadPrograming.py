...
def problem3(fileName):
    ...
    while not sepNum.isdigit():
         sepNum = input('Enter the number of processes: ')
    sepNum = int(sepNum)
    sep_list = GetSepList(fileName, sepNum)
    # get the processed sepratedly list
    ...
    # clock start
    for i in range(sepNum):
        p = multiprocessing.Process(name='bb_process-' + str(i + 1), target=BubbleSort, args=(sep_list[i], q))
        p.start()
        processes.append(p)
    ...
    while cnt != sepNum - 1:
        if q.qsize() >= 2:
            ll = q.get()
            rl = q.get()
            mp = multiprocessing.Process(name='m_process-' + str(cnt + 1), target=MergeSort, args=(ll, rl, q))
            mp.start()
...
if __name__ == '__main__':
    ...
    c = input('Which case do you want ?')
